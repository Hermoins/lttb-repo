...
anomaly_mask[1200:1250] = True; anomaly_mask[1800:2150] = True; anomaly_mask[2700:2800] = True
normal_mask = ~anomaly_mask

norm_fix = np.sum(normal_mask[idx_fix]); anom_fix = np.sum(anomaly_mask[idx_fix])
norm_dyn = np.sum(normal_mask[idx_dyn]); anom_dyn = np.sum(anomaly_mask[idx_dyn])

fig, axes = plt.subplots(3, 1, figsize=(14, 12))

# 附图 1：原始数据
axes[0].plot(t, data, 'k-', alpha=0.5, label='原始流式数据 (1Hz, 3600点)')
axes[0].set_title('附图 1：原始矿井瓦斯浓度数据 (包含三种典型异常与高频噪声)', fontsize=13, fontweight='bold')
axes[0].legend(loc='upper left'); axes[0].grid(alpha=0.3); axes[0].set_ylim(0, 2.5)
...
axes[1].set_title('附图 2：传统固定桶 LTTB (资源分配死板：在平稳区浪费带宽，在危险区采样严重不足)', fontsize=13, fontweight='bold')
axes[1].legend(loc='upper left'); axes[1].grid(alpha=0.3); axes[1].set_ylim(0, 2.5)
...
axes[2].set_title('附图 3：本发明“方差双阈值”动态截断 LTTB (智能算力挪用：将平稳区的算力“挪用”到危险区高频包抄)', fontsize=13, fontweight='bold')
axes[2].legend(loc='upper left'); axes[2].grid(alpha=0.3); axes[2].set_ylim(0, 2.5)
axes[2].set_xlabel('时间 (秒)', fontsize=12)

plt.tight_layout()
plt.savefig('patent_figures_1_to_3.png', dpi=300, bbox_inches='tight')
print("✅ 【附图 1~3】已生成并保存为 patent_figures_1_to_3.png")
plt.show()